...
;;; by means of Monte Carlo simulation. Consider computing the area of a region of
...
;;; with diagonally opposite corners at (2, 4) and (8, 10) contains the circle above.
;;; The desired integral is the area of
...
;;; takes as arguments a predicate P, upper and
;;; lower bounds x1, x2, y1, and y2 for the rectangle, and the
;;; number of trials to perform in order to produce the estimate.
;;; Your procedure should use the same monte-carlo
;;; procedure that was used above to estimate π.
;;; Use your estimate-integral to produce an estimate of π by measuring the
;;; area of a unit circle.
;;; You will find it useful to have a procedure that returns a
;;; number chosen at random from a given range. The following random-in-range
;;; procedure implements this in terms
...
(define (estimate-integral predicate x1 y1 x2 y2)
  (define (test) (predicate (random-in-range x1 x2) (random-in-range y1 y2)))
  (* (monte-carlo test 10000000) (* (- x2 x1) (- y2 y1))))
