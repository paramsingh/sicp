...
(pretty-print "tests for position")
(position 0 0 0 0)
(position 0 0 20 0)
(position 0 5 10 10)
(position 2 2 2 2)
...
(define time-to-impact
  (lambda (vertical-velocity elevation)
    (define r1 (root1 (* (/ 1 2) gravity) (- vertical-velocity) (- elevation)))
    (define r2 (root2 (* (/ 1 2) gravity) (- vertical-velocity) (- elevation)))
    (if (> r1 0) r1 r2)
    ))

(pretty-print "test for time-to-impact")
...
(define time-to-height
  (lambda (vertical-velocity elevation target-elevation)
    (root1 (* (/ 1 2) gravity) (- vertical-velocity) (- target-elevation elevation)))) ;;

(pretty-print "test for time-to-height")
(time-to-height 10 100 5)
...
(define travel-distance-simple
  (lambda (elevation velocity angle)
    (define hv (* velocity (cos (degree2radian angle))))
    (define vv (* velocity (sin (degree2radian angle))))
    (define t (time-to-impact vv elevation))
    (* hv t)))
...
(println "let us calculate time to impact")
(time-to-impact (* (sin (degree2radian 0)) 45) 1)
(time-to-impact (* (sin (degree2radian 90)) 45) 1)
(time-to-impact (* (sin (degree2radian 45)) 45) 1)

;;; ;; what is the distance traveled in each case?
;;; ;; record both in meters and in feet

(println "==================distances in m===================")
(travel-distance-simple 1 45 0)
(travel-distance-simple 1 45 90)
(travel-distance-simple 1 45 45)

(println "==================distances in feet===================")
...
(define find-best-angle
  (lambda (velocity elevation)
    (argmax (lambda (angle) (travel-distance-simple elevation velocity angle)) (range 0 91))))

;;; ;; find best angle
(println "==================finding best angle====================")
(find-best-angle 45 1)
(println "==================finding best angle (increasing v) ====================")
;;; ;; try for other velocities
(find-best-angle 45 10)
(find-best-angle 45 50)
(find-best-angle 45 100)
(find-best-angle 45 10000)
(println "==================finding best angle (decreasing h) ====================")
;;; ;; try for other heights
(find-best-angle 30 1)
(find-best-angle 25 1)
...
(define integrate
  (lambda (x0 y0 u0 v0 dt g m beta)
    (define (integrate-helper x y u v)
      (define dx (* u dt))
      (define dy (* v dt))
      (define speed (sqrt (+ (* u u) (* v v))))
      (define du (- (* (/ 1. m) speed beta u dt) ))
      (define dv (- (* (+ g (* (/ 1. m) speed beta v)) dt)))
      (cond ((<= y 0) x)
            (else (integrate-helper (+ x dx) (+ y dy) (+ u du) (+ v dv))))
      )
    (integrate-helper x0 y0 u0 v0)
    ))
...
(define (travel-distance elevation velocity angle)
  (define u0 (* velocity (cos (degree2radian angle))))
  (define v0 (* velocity (sin (degree2radian angle))))
  (integrate 0 elevation u0 v0 0.1 gravity mass beta)
  )

;;; ;; RUN SOME TEST CASES

(println "=================travel distance with drag==============")
